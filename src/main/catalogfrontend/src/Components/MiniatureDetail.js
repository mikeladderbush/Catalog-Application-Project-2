...
const MiniatureDetail = () => {
  ...
  const loadData = async () => {
    try {
      const response = await axios.get(`http://localhost:8080/api/v1/miniature-controller/${token}`, {
  ...
  const handleUpdate = () => {
    ...
    if (miniature.miniatureId) {
      axios.put(`http://localhost:8080/api/v1/miniature-controller/${token}`, dataToUpdate)
        .then((response) => {
      ...
      axios.post(`http://localhost:8080/api/v1/miniature-controller/${token}`, dataToUpdate)
        .then((response) => {
